library(DPP)
setwd("/home/lavila/projects/282inversions/1MB_windows/chr3")

#cat("task is:",Sys.getenv("MYTASK"),"\n",file="taskLogger.log",append=TRUE)
task.id<-Sys.getenv("MYTASK")
system(paste("mkdir ","outs/window_",task.id,sep=""))
window.dir<-paste("./outs/window_",task.id,sep="")
file.name.prefix<-paste("c3_win_",task.id,sep="")
input.file.name<-paste("./",file.name.prefix,".pcs",sep="")
pcs<-read.table(input.file.name,sep="\t")
y<-pcs[,1]
y <- c(y,rnorm(1,0,0.1))
y<-y[1:(length(y)-1)]
...
for(i in 1:num_runs) {
 ...
 my_analysis_withc <- dppMCMC_C(
  ...
  my_analysis_withc$run(generations,auto_stop=TRUE,max_gen = max_generations,min_ess = 500)
