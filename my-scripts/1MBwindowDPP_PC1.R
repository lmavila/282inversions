...
path.prefix="/home/lavila/projects/282inversions/1MB_windows/chr"

task.id<-Sys.getenv("MYTASK")
chr<-Sys.getenv("CHR") 
#cat("task is:",task.id,"\tchr is: ",chr,"\n",file="taskLogger.log",append=TRUE)

setwd(paste0("/home/lavila/projects/282inversions/1MB_windows/chr",chr))
...
concentration_parameter_alpha= 1.080173 #from whole genome estimation of chi_G and fitting
concentration_parameter_beta=10;

# mcmc settings
generations <- 1000
expectedClusters <- 5.054454 #from whole genome estimation with DPP
num_runs <-1
